 let userData = {
    ...
    function renderProfile() {
      ...
      document.getElementById('display-birthdate').textContent = formatDate(userData.birthdate);
    ...
    function renderAddresses() {
      ...
      userData.addresses.forEach(address => {
        ...
        addressCard.innerHTML = `
          ${address.isPrimary ? '<div class="address-primary">Rumah Utama</div>' : ''}
          <div class="address-name">${address.name}</div>
          <div class="address-phone">${address.phone}</div>
          <div class="address-detail">${address.address}</div>
          ${address.note ? `<div class="address-note">${address.note}</div>` : ''}
          <div class="address-actions">
            <div class="address-action" onclick="setPrimaryAddress(${address.id})">${
              address.isPrimary ? 'Alamat Utama' : 'Jadikan Utama'
            }</div>
            <div class="address-action" onclick="openEditAddressForm(${address.id})">Ubah Alamat</div>
            ${!address.isPrimary ? `<div class="address-action" onclick="deleteAddress(${address.id})">Hapus</div>` : ''}
          </div>
        `;
    ...
    function openEditAddressForm(id) {
      ...
      const content = `
        <div class="form-group">
          <label class="form-label">Label Alamat</label>
          <input type="text" class="form-input" id="address-label" value="${address.label}">
        </div>
        <div class="form-group">
          <label class="form-label">Nama Penerima</label>
          <input type="text" class="form-input" id="address-name" value="${address.name}">
        </div>
        <div class="form-group">
          <label class="form-label">Nomor HP</label>
          <input type="tel" class="form-input" id="address-phone" value="${address.phone}">
        </div>
        <div class="form-group">
          <label class="form-label">Alamat Lengkap</label>
          <textarea class="form-input" id="address-detail" rows="3">${address.address}</textarea>
        </div>
        <div class="form-group">
          <label class="form-label">Catatan Untuk Kurir (Opsional)</label>
          <textarea class="form-input" id="address-note" rows="2">${address.note}</textarea>
        </div>
        <div class="form-group">
          <label class="form-label">
            <input type="checkbox" id="address-primary" ${address.isPrimary ? 'checked disabled' : ''}>
        ...
        <div class="form-actions">
          ...
          ${!address.isPrimary ? `<button class="btn btn-danger" onclick="deleteAddress(${id})">Hapus</button>` : ''}
        </div>
      `;
      openForm(title, content);
    }
    ...
    function deleteAddress(id) {
      if (confirm('Apakah Anda yakin ingin menghapus alamat ini?')) {
        userData.addresses = userData.addresses.filter(addr => addr.id !== id);
        saveData();
      }
    }